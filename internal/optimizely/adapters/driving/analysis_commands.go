...
func promoteFlags(s *core.Service, config *Config, writer printer.Writer) *cobra.Command {
  ...
  satt optimizely flags promote --project 123 --lower-env development --lower-env qa
`),
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx, cfg, err := analysisContextAndConfig(cmd, config, skipCacheFlag)
			...
			var result []core.PromotionCandidate
			err = loader.Run("Finding promotion candidates...", func() {
				result, err = s.FindPromotionCandidates(ctx, targetProjectID, againstProjects, core.PromotionOptions{
					Query:                query,
					LowerEnvironmentKeys: lowerEnvs,
				})
			})
			if err != nil {
				return err
			...
			return writeAnalysisOutput(tui.RenderOptimizelyPromotionCandidates(result, targetProjectID, againstProjects), toFilePath, stdout)
		},
	}
